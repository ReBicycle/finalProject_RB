...
public class LoginCheckInterceptor extends HandlerInterceptorAdapter{
	/**
	 * 해당 컨트롤러 수행전에 실행(제어) 되는 메소드
	 * preHandle 메소드에서 ture를 반환하면 기존 컨트롤러가 실행하고
	 * false 를 반환하면 기존 컨트롤러는 실행되지 않는다
	 * 
	 * 인증여부 화인하여 인증되어 있으면 true 를 반환
	 * 인증되어 있지 않으면
	 * index로 리다이렉트후
	 * false 반환
	 */
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		System.out.println("**핸들러인터셉터실행**"+request.getRequestURI());
